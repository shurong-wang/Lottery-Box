    ...
    <div class="lucky-draw-view">
      ...
      <div :class="isLuckyDraw ? 'lucky-draw-content lucky-draw-start' : 'lucky-draw-content'">
        <div :class="isLuckyDraw ? 'lucky-draw-users lucky-draw-users-start' : 'lucky-draw-users'">
          ...
          <div v-if="!users.length && !surplusUsers.length" class="ucky-draw-empty">Congratulations, see you next time.</div>
        </div>
      </div>
      <!-- 设置奖项，人数，并开始抽奖 -->
      <div class="lucky-draw-tool-left">
        ...
        <a-input
          ...
          placeholder="Number of people drawn in this round"
        />
        <!-- 抽奖按钮 -->
        <a-button @click="luckyDraw">
          {{ isLuckyDraw ?  luckyDrawTime ? 'Stop the draw' : 'Ending the round' : 'Start the draw' }}
        </a-button>
      </div>
      <!-- 右边工具栏 -->
    ...
    luckyDraw () {
      ...
      if (this.isLuckyDraw) {
        ...
        if (this.numberPeople <= 0) {
          this.$message.error('The number of raffle winners must be greater than 0')
          return
        }
        if (this.numberPeople > users.length) {
          this.$message.error(`There are ${users.length} people on the draw list, the number of people filling out the draw must be less than or equal to ${users.length}`)
          return
        }
        // 开始抽奖
        this.startLuckyDraw()
